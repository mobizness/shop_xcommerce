...
class Category extends Model{
	...
	public function copyCategory($category_id){
		...
		if($category){
			DB::insert(
				'insert into categories (title, image, alt_text, image2, alt_text2, short_description, order_no, parent_id, status, modifydate, createdate) 
				values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', 
				[$category->title, $category->image, $category->alt_text, $category->image2, $category->alt_text2, $category->short_description, $category->order_no, $category->parent_id, $category->status, date('Y-m-d H:i:s'), date('Y-m-d H:i:s')]
			);
				...
				$html .= '<li>
						<div style="font-size:17px; display: block; height: 30px; margin: 5px 0; padding: 5px 10px; text-decoration: none; border: 1px solid #ccc; background: #fafafa; box-sizing: border-box;">
							...
							<div class="pull-right">
								<a href="' . url('web88cms/categories/category_home_list') . '" title="Edit"><span class="label label-sm label-success"><i class="fa fa-pencil"></i></span></a>
							</div>
						</div>
					</li>';
			}
			...
			foreach($categories as $category){
            	$html .= '<li data-id="' . $category['category_id'] . '" class="dd-item">';
                	$html .= '<div class="dd-handle">';
						$html .= '<span id="text-category-' . $category['category_id'] . '">' . $category['title'] . '</span>';
	...
	{
		$res= DB::table('category_home_list_tablisting')->get();
		return $res;
	}
	/*******************************************************************************************************/
	...
	function getProductsforcategory_homeproduct()
	{
		...
		if(isset($_GET['catid'])&&$_GET['catid']!='')
		{
			...
			FROM
				products p
				JOIN product_to_category pcat  ON pcat.product_id = p.id
				JOIN product_to_color pcol ON pcol.product_id = p.id 
				 JOIN colors col ON col.id = pcol.color_id
			...
			FROM
				products p
				JOIN product_to_category pcat  ON pcat.product_id = p.id
				JOIN product_to_color pcol ON pcol.product_id = p.id && pcat.category_id='".$_GET['catid']."'
				 JOIN colors col ON col.id = pcol.color_id
			   
			WHERE
				p.status='1'
				
				group by p.id,pcat.category_id 
				");
			}
					  	...
					  	$i = 1;
                            foreach($products as $details)
                            {
								...
								$status = ($details->status == '0') ? 'In-active' : 'Active'; 
                                             ...
                                             echo '<tr>
                                                        <td><input type="checkbox"  value="'.$details->id.'" data-id="'.$details->id.'" onclick="selectForPost();" class="select_items"/></td>
...
/*******************************************/	
	...
	function list_selected_producted($start_from, $num_rec_per_page,$tabid,$homecatid)
	{
		...
		FROM
			...
			JOIN product_to_color pcol ON pcol.product_id = p.id
			 JOIN colors col ON col.id = pcol.color_id
			  JOIN category_home_addtabproductsdata chome ON chome.productid = p.id
	...
	function search_list_selected_producted($start_from, $num_rec_per_page,$search_for,$tabid,$homecatid)
	{
		...
		FROM
			...
			JOIN product_to_color pcol ON pcol.product_id = p.id
			 JOIN colors col ON col.id = pcol.color_id
			  JOIN category_home_addtabproductsdata chome ON chome.productid = p.id WHERE
