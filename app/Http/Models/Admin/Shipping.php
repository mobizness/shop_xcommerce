...
class Shipping extends Model{
	...
	function addShippingOrderAmount($formData)
	{
		...
		DB::table('shipping_order_amounts')->insert($formData);
		$shipping_order_id = DB::getPdo()->lastInsertId();
		
	}
	
	// get all records
	function getShippingOrderAmount()
	{
		$per_page = (Session::has('global_discounts.per_page')) ? Session::get('global_discounts.per_page') : 30;
		return DB::table('shipping_order_amounts')->paginate($per_page);	
	}
...
/*	function getTotalProducts($current_page)
	...
	function deleteShippingOrderAmount($item_id)
	{
		// delete global_discounts
		DB::table('shipping_order_amounts')->whereIn('id',explode(',',$item_id))->delete();
	...
	function updateShippingOrderAmount($formData)
	{
		unset($formData['_token']);
		...
		DB::table('shipping_order_amounts')->where('id',$shipping_order_amount_id)->update($formData);
