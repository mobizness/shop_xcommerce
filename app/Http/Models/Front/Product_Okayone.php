...
class Product extends Model{
	...
	*/
	...
	function getProductBanners($category_id = 0)
	{
		...
		$results = DB::table('product_banner_list')->where('tick','1')->where('status','1')->whereIN('category', $categoryList)->get();
	...
	public function getCartProducts($cartProducts, $promocode = array(), $estimateShipping = array()){
		$productsInfo = array();
		$shippingTotal = 0;
		$totalPrice = 0;
		...
		if($cartProducts){
			...
			foreach($cartProducts as $key => $cartProduct){
				...
				if($product){
					...
					if(!$product->free_shipping && $product->shipping_cost){
						if(isset($estimateShipping['shipping_method']) && $estimateShipping['shipping_method'] == 'Self Collection')
						{
							$productsInfo[$i]->shipping_cost = 0;
						}
						else
						{
					...
					if($product->is_tax)
					{
						//$productsInfo[$i]->gst = ($product->sale_price / 100) * 6;
						//$totalPrice += $cartProduct['quantity'] * $productsInfo[$i]->gst;
						$productsInfo[$i]->gst = 0;
					}
					else
					{
						$productsInfo[$i]->gst = 0;
			...
			foreach($productsInfo as $key => $product){
				...
				if($quantityDiscount){
					if($quantityDiscount->discount_by == 'percentage'){
						$productsInfo[$key]->cart['quantityDiscount'] = number_format((($quantityDiscount->discount/100) * $product->sale_price), 2);
					}
					else{
			...
			foreach($productsInfo as $key => $product){
				$globalDiscount = $this->getGlobalDiscount($product->id, ($totalPrice + $shippingTotal));
				...
				elseif($globalDiscount && $globalDiscount->discount_by == 'percentage'){
					$salePrice = $product->sale_price - $product->cart['quantityDiscount'];
					...
					foreach($productsInfo as $key => $value){
						...
						if($isApplicable){
							...
							if($promocodeInfo->discount_type == 'P'){
								$salePrice = $value->sale_price - $value->cart['quantityDiscount'] - $productsInfo[$key]->cart['globalDiscount'];
	...
	*/
	...
	{
		//SELECT c.id,c.title FROM categories c left join `product_to_category` pc ON pc.category_id=c.id where product_id = 305
		return DB::table('categories as c')->select('c.id','c.title')->leftJoin('product_to_category as pc','pc.category_id','=','c.id')->where('product_id',$product_id)->get();
	}
}