	...
	public function topbanner()
   	{
               ...
               $imageName = null;
				...
				if(isset($_FILES['enlarge_banner']['name']) && $_FILES['enlarge_banner']['name']!='')
				{
					...
					Request::file('enlarge_banner')->move(
						base_path() . '/public/admin/images/banner/top', $enlargebannerdata
					);
				}	
				...
				{
					if($_FILES['pdf_link']['size']==0)
					{
						 return Redirect::to('web88cms/index_banner_top_list')->withInput()->with('error','PDF file size exceeds its limit. Please try again..');
					}
					else
					{
					$pdf_link_data = time().'_'.$_FILES['pdf_link']['name'];
					Request::file('pdf_link')->move(
						base_path() . '/public/admin/images/banner/top', $pdf_link_data
					);
					}
				}  
		...
		$banner =$imageName; 
		$enlarge_banner = $enlargebannerdata;
		$pdf_link = $pdf_link_data;
		$modified = date('Y-m-d H:i:s');
			$created = date('Y-m-d H:i:s'); 
		DB::table('banner_top')->insert(array(
		array('status' => $status, 'token' => $token, 'title' => $title,'banner' => $banner,'start_date' => $start_date,'end_date' => $end_date,'display_order' => $display_order,
		'enlarge_banner' => $enlarge_banner,'pdf_link' => $pdf_link, 'url' => $url,'created' => $created, 'modified' => $modified) ));
